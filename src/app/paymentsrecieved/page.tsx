...
const GetStarted = () => {
  const [recievedPayments, setRecievedpayments] = useState<IInvoice[]>([]);

  useEffect(() => {
    handleGetRecievedPayments()
      .then(({ data }: any) => {
        setRecievedpayments(data.recievedPayments);
      })
      .catch((err) => {
        console.log(err);
        alert(err);
          ...
          <div className="w-full flex flex-col items-center">
            <div className="p-5 flex justify-between w-full">
              <div className="text-white">
                <h2 className="text-xl">All Recieved Payments</h2>
              </div>
            ...
            <table className="w-full border-t border-white">
              ...
              <tbody className="text-white text-sm">
                {recievedPayments.map((item: IInvoice) => (
                  <tr className="border-b border-white" key={item.id}>
                    <td className="p-2 text-center text-secondary">PAID</td>
                    ...
                    <td className="p-2 text-center">{item.paymentMode}</td>
                    <td className="p-2 text-center">₹ {item.Total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {recievedPayments.length == 0 && (
              <WarningMessage href="/createnewinvoice" warningWord="Recieved Payments" />
            )}
          </div>
        </div>
      </div>
